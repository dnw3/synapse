...
impl AgentSession {
    ...
    /// Deep Agent mode with streaming via RunContext.
    ///
    /// Like `handle_deep_agent_streaming` but uses `stream_with_context` to
    /// propagate the RunContext (which carries the StreamingOutputHandle and
    /// cancel token) through the graph execution, enabling the framework's
    /// `StreamingInterceptor` to forward tokens automatically.
    pub(super) async fn handle_deep_agent_streaming_with_context(
        &self,
        session_id: &str,
        text: &str,
        content_blocks: &[ContentBlock],
