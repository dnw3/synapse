...
pub async fn handle_list(ctx: Arc<RpcContext>, _params: Value) -> Result<Value, RpcError> {
    ...
    agents.push(json!({
        "name": "default",
        "id": "default",
        "model": ctx.state.core.config.model_config().model,
        "system_prompt": ctx.state.core.config.agent_config().system_prompt,
        "channels": [],
        "is_default": true,
    ...
    for agent_def in &effective.list {
        ...
        agents.push(json!({
            ...
            "model": agent_def.model.clone().unwrap_or_else(|| ctx.state.core.config.model_config().model.clone()),
...
pub async fn handle_create(ctx: Arc<RpcContext>, params: Value) -> Result<Value, RpcError> {
    ...
    Ok(json!({
        "name": name,
        "model": model.unwrap_or(&ctx.state.core.config.model_config().model),
