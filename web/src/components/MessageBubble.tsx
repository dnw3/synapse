...
function LogIdBadge({ requestId }: { requestId: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full text-[10px] font-mono text-[var(--text-tertiary)] bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] hover:border-[var(--accent)]/30 hover:text-[var(--text-secondary)] transition-all cursor-pointer select-none"
    >
      {copied ? (
  ...
  if (message && message.role === "human") {
    return (
      <div className="flex gap-3 justify-end animate-fade-in">
        ...
        <div className="w-7 h-7 rounded-full bg-[var(--bg-surface)] border border-[var(--border-default)] flex items-center justify-center flex-shrink-0">
